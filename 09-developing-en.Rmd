...
2. **DO NOT** delete slides. To hide a slide for remote instruction, use the `instruction: remote` tag. See an example below:
...
Workshops should be interactive and should include exercises and polls. Below, we provide a few exercise examples evidence-based learning modes and effective strategies to deliver remote workshops to participants. 
...
Context: Workshop 7 participants have just learned about `quasipoisson` family objects and how they can help to account for overdispersion in their poisson models. They were also introduced to the possibility of applying an overdispersion correction for binary data with the `quasibinomial` family object.
...
```
---
We now realized that the proportion example for infected deer may have some issues with overdispersion. Which of the following functions would be an appropriate update to account for overdispersion in our proportion data?
...
---
We now realized that the proportion example for infected deer may have some issues with overdispersion. Which of the following functions would be an appropriate update to account for overdispersion in our proportion data model?
...
Context: We are reaching the end of Workshop 9 and the presenter has just explained the summary table showing the main characteristics of each type of ordination we learnt during the workshop (https://wiki.qcbs.ca/_media/summary_ordination.jpg?cache=).
