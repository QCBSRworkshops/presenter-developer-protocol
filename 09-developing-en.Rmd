...
Current implementation uses Travis CI and is projected to be shifted towards Github Actions.

In a shell, inside each repository there is a `.github/workflows/` directory that holds `YAML` files that contain a workflow recipe that tells Github Actions what to run and when to run it.

Below, we will show how to create actions to deploy books built using `bookdown` (such as this oen and the one for the upcoming workshop material) and to deploy presentations built using `rmarkdown`.
